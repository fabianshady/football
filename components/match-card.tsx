  ...
  return (
    <Card className={`overflow-hidden hover-lift border-l-4 ${borderColor} glass-card group`}>
      <CardContent className="p-4">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-6 mt-4 pt-3 border-t border-border/30 text-sm text-muted-foreground">
          ...
          {match.kit && (match.kit === 1 || match.kit === 2) && (
            <div className="flex items-center gap-2">
              <Image
                ...
                alt={`Uniforme ${match.kit === 1 ? 'Local' : 'Visitante'}`}
                width={18}
                height={18}
                className="object-contain filter drop-shadow-sm"
              />
              <span>
                Uniforme: <span className="font-semibold text-foreground">{match.kit === 1 ? 'Local (Azul)' : 'Visitante (Guinda)'}</span>
              </span>
            </div>
          )}
        </div>
